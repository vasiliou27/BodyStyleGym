...
export default function Navigation() {
  ...
  return (
    ...
    >
      ...
      >
        <div className="px-4 pt-2 pb-4 space-y-2 bg-dark/98 backdrop-blur-sm">
          {navLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
          ...
          <Link
            href="/memberships"
            onClick={() => setIsOpen(false)}
            className="block px-4 py-3 bg-primary text-dark font-bold rounded-md text-center hover:bg-primary-light transition-all duration-300"
          >
            {t.nav.joinNow}
          </Link>
        </div>
